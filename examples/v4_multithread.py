# Running the threaded evaluation code as provided

import threading
from collections import deque
import sys

# Updated Python port with locks and threaded test harness
...
_run_lock = threading.Lock()  # Ensure one evaluation at a time for reset+eval

def set_subst(name: int, term):
    with _gSUBST_lock:
        _gSUBST[name] = term

def get_subst(name: int):
    with _gSUBST_lock:
        return _gSUBST.pop(name, None)
...
class Dup(Term):
    ...
    def __str__(self):
        l = self.label; xs = name_str(self.x); ys = name_str(self.y)
        if l == 0:
...
class Lam(Term):
    ...
    def __str__(self):
        l = self.label; xs = name_str(self.x)
        if l == 0:
...
def app_sup(f: Term, arg: Term, app_lab: int):
    if isinstance(f, Sup):
        inc_inters(); mark_reduction()
        lab = f.label; left = f.left; right = f.right
        c0 = fresh(); c1 = fresh()
        a0 = App(app_lab, left, Var(c0)); a1 = App(app_lab, right, Var(c1))
...
def app_dup(term: Term):
    if isinstance(term, App) and isinstance(term.arg, Dup):
        app_lab = term.label; f = term.func; dup: Dup = term.arg
        inc_inters(); mark_reduction()
...
def dup_era(dup_term: Dup, v: Term):
    if isinstance(v, Era):
        inc_inters(); mark_reduction()
        set_subst(dup_term.x, Era()); set_subst(dup_term.y, Era())
...
def dup_lam(dup_term: Dup, v: Term):
    if isinstance(v, Lam):
        inc_inters(); mark_reduction()
        lam_lab = v.label; x = v.x; f = v.body
        x0 = fresh(); x1 = fresh(); f0 = fresh(); f1 = fresh()
...
def dup_sup(dup_term: Dup, v: Term):
    if isinstance(v, Sup):
        inc_inters(); mark_reduction()
        sup_lab = v.label; a = v.left; b = v.right
        if dup_term.label == sup_lab:
            set_subst(dup_term.x, a); set_subst(dup_term.y, b)
            return whnf(dup_term.body)
        else:
            a0 = fresh(); a1 = fresh(); b0 = fresh(); b1 = fresh()
...
def whnf(term: Term) -> Term:
    if isinstance(term, Var):
        ...
        if has_reduced():
            ...
            set_subst(term.name, v_whnf); mark_reduction()
            return whnf(term.t2)
...
class Parser:
    def __init__(self, text: str):
        self.text = text; self.pos = 0; self.len = len(text); self.global_map = {}
    ...
    def getGlobalName(self, gname: str) -> int:
        if gname in self.global_map:
            ...
            n = fresh(); self.global_map[gname] = n; return n

    def bindVar(self, name: str, ctx: dict) -> (int, dict):
        if self.isGlobalName(name):
            n = self.getGlobalName(name); return n, ctx
        else:
            n = fresh(); ctx2 = ctx.copy(); ctx2[name] = n; return n, ctx2
    ...
    def parseTerm(self, ctx: dict) -> Term:
        self.skip_whitespace_and_comments(); pos0 = self.pos
    ...
    def parseSimpleTerm(self, ctx: dict) -> Term:
        ...
        if self.pos < self.len and self.current() == '*':
            ...
            self.expect_string('('); t = self.parseTerm(ctx); self.expect_string(')'); return t
        else:
            return self.parseVar(ctx)

    def parseVar(self, ctx: dict) -> Term:
        name = self.parseVarName(); n = self.getVar(name, ctx); return Var(n)

    def parseEra(self) -> Term:
        self.expect_string('*'); return Era()
    ...
    def parseLam(self, ctx: dict) -> Term:
        ...
        if self.match_string("&"):
            try:
                ...
                varname = self.parseVarName(); n, ctx2 = self.bindVar(varname, ctx)
                self.expect_string("."); body = self.parseTerm(ctx2)
    ...
    def parseApp(self, ctx: dict) -> Term:
        ...
        if self.match_string("&"):
            try:
                lab = self.parse_natural()
                self.expect_string("("); f = self.parseTerm(ctx); self.skip_whitespace_and_comments()
                a = self.parseTerm(ctx); self.expect_string(")")
                return App(lab, f, a)
            except ParseError:
                self.pos = pos0
        if self.match_string("("):
            f = self.parseTerm(ctx); self.skip_whitespace_and_comments()
            a = self.parseTerm(ctx); self.expect_string(")")
            return App(0, f, a)
        if self.match_string("["):
            f = self.parseTerm(ctx); self.skip_whitespace_and_comments()
    ...
    def parseSup(self, ctx: dict) -> Term:
        ...
        if self.match_string("&"):
            try:
                lab = self.parse_natural(); self.expect_string("{")
                a = self.parseTerm(ctx); self.expect_string(","); b = self.parseTerm(ctx); self.expect_string("}")
                return Sup(lab, a, b)
            except ParseError:
                self.pos = pos0
        if self.match_string("{"):
        ...
        if self.match_string("!"):
            ...
            if self.match_string("&"):
                try:
                    lab = self.parse_natural()
                    if self.match_string("{"):
                        name1 = self.parseVarName(); self.expect_string(","); name2 = self.parseVarName()
                        self.expect_string("}"); self.expect_string("=")
                        val = self.parseTerm(ctx); self.expect_string(";")
                        n1, ctx1 = self.bindVar(name1, ctx); n2, ctx2 = self.bindVar(name2, ctx1)
                        body = self.parseTerm(ctx2); return Dup(lab, n1, n2, val, body)
                    else:
                ...
                if self.match_string("{"):
                    ...
                    val = self.parseTerm(ctx); self.expect_string(";")
                    n1, ctx1 = self.bindVar(name1, ctx); n2, ctx2 = self.bindVar(name2, ctx1)
                    body = self.parseTerm(ctx2); return Dup(0, n1, n2, val, body)
                self.pos = pos1
                if self.match_string("<"):
                    name1 = self.parseVarName(); self.expect_string(","); name2 = self.parseVarName()
                    self.expect_string(">"); self.expect_string("=")
                    val = self.parseTerm(ctx); self.expect_string(";")
                    n1, ctx1 = self.bindVar(name1, ctx); n2, ctx2 = self.bindVar(name2, ctx1)
                    body = self.parseTerm(ctx2); return Dup(1, n1, n2, val, body)
        raise ParseError("Not a Dup")

    def parseLet(self, ctx: dict) -> Term:
        self.skip_whitespace_and_comments(); pos0 = self.pos
        if self.match_string("!"):
            name_token = self.parseVarName(); self.expect_string("=")
            t1 = self.parseTerm(ctx); self.expect_string(";")
            n, ctx2 = self.bindVar(name_token, ctx)
            t2 = self.parseTerm(ctx2); return Let(n, t1, t2)
        self.pos = pos0
...
# Worker using locks and shared globals
def worker(expr: str, thread_id: int):
    global _gINTERS, _gSUBST, _gFRESH, _gSTOP
    # Ensure reset+eval atomic
    with _run_lock:
        # reset globals
        with _gINTERS_lock:
            _gINTERS = 0
        ...
        term = doParseIC(expr)
        print(f"[Thread {thread_id}] Initial term and reductions:")
        result = normal(term)
        inters = read_inters()
        print(f"[Thread {thread_id}] Final normal form: {result}")
        print(f"[Thread {thread_id}] - WORK: {inters}")
...
# Launch threads
threads = []
for i in range(2):  # two threads running same test
    t = threading.Thread(target=worker, args=(expr, i))
    t.start()
    threads.append(t)
